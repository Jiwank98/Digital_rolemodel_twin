...
class Model(nn.Module):
    ...
    def __init__(self, action_num):
        ...
        self.rf_dim = 8 *4
        self.action_dim = 1
        self.input_dim = self.rf_dim + self.action_dim
...
class Replay_Buffer:
    ...
    def sample(self, batch_size):
        ...
        batch_rf,batch_action, batch_reward, batch_rf_new, batch_action_new = [], [], [], [], []

        for transition in batch:
            rf,a,r,rf_prime,action_prime = transition
            batch_rf.append(rf)
        ...
        return torch.tensor(np.array(batch_rf), dtype=torch.float), torch.tensor(np.array(batch_action),dtype=torch.float), torch.tensor(np.array(batch_reward),dtype=torch.float),torch.tensor(np.array(batch_rf_new),dtype=torch.float), torch.tensor(np.array(batch_action_new),dtype=torch.float)
...
class Agent:
    ...
    def update_Q_network(self, rf, action, reward, rf_new, action_new):
        ...
        state = []
        state = torch.Tensor(rf)
        state_new = torch.Tensor(rf_new)
    ...
    def take_action(self, rf,action_num): #엡실론 사용해서 random뽑기 / 나머지 확률로 높은거 뽑기
        state = rf
    ...
    def next_take_action(self,next_rf, action_num):
        next_state = next_rf
        next_state = torch.from_numpy(next_state).float()
        next_action = np.zeros(8)
        for i in range(0,8):
          next_action[i]= i
...
def kakaoFeedback(recommended_news1,recommended_news2,recommended_news3,user): #선호도 만들기
  favor_news=[1]*8
...
def s_sp(n_epi,all):
  ...
  for i in range(8): #all/epoch
    S_P[i]= sum1[i]

  for i in range(8): #3개월
    S_P[8+i]=sum2[i]

  for i in range(8): #한달
    S_P[16+i]=sum3[i]

  for i in range(8): #일주일
    S_P[24+i]=sum4[i]
...
def load_users(epochs):
  ...
  return users,users_rf,all_list,max_q_list, reward_list

def run(users, users_rf,all_list, n_epi,answer,recommend,agent,buffer,avg_q_list, reward_list,precision,recall):
  n=len(users)
  action1=[0]*n
  action2 = [0] *n
  action3 = [0] * n
...
#유저별로 추천하는 뉴스 만들기
  for user in range(n):
    rf = users_rf[user]
    action1[user],action2[user], action3[user], avg_q_value[user] = agent.take_action(rf,agent.action_number)
...
#유저별로 kakao톡에 추천 뉴스 보내주기
  for user in range(n):
    isnewsread[user] = kakaoFeedback(action1[user],action2[user],action3[user],users[user])
...
#유저별로 카카오톡에서 읽은 뉴스들 카테고리 가져와서 학습돌리기
  for user in range(n):
    ...
    if user==0: #0, 1선호
        if n_epi % 4==3:
            if action1[user] == 0 or action2[user] == 0 or action3[user] == 0:
                pr += 1
                rc+=1
            if action1[user] == 1 or action2[user] == 1 or action3[user] == 1:
                pr += 1
                rc+=1
            if action1[user] == 4 or action2[user] == 4 or action3[user] == 4:
                pr += 1
                rc+=1
            precision[user][n_epi] = pr/3
            recall[user][n_epi] = pr/3
            ...
            if action1[user] == 0 or action2[user] == 0 or action3[user] == 0:
                ...
                rc+=1
            precision[user][n_epi]= pr/3
            recall[user][n_epi] = pr /2


    if user == 1:  # 2,3선호
        if n_epi % 4 == 3:
            if action1[user] == 2 or action2[user] == 2 or action3[user] == 2:
                pr += 1
                rc+=1
            if action1[user] == 3 or action2[user] == 3 or action3[user] == 3:
                pr += 1
                rc+=1
            if action1[user] == 4 or action2[user] == 4 or action3[user] == 4:
                pr += 1
    ...
    next_action= agent.next_take_action(S_P, agent.action_number)

    buffer.append((users_rf[user],action1[user],a_user1_action1_reward,S_P,next_action))
    buffer.append((users_rf[user],action2[user],a_user1_action2_reward,S_P,next_action))
    buffer.append((users_rf[user], action3[user], a_user1_action3_reward, S_P, next_action))
    loss=0
    if buffer.size() > 32:
      batch_rf, batch_action, batch_reward, batch_rf_new, batch_action_new = buffer.sample(Config.batch_size)
      loss= agent.update_Q_network(batch_rf, batch_action, batch_reward, batch_rf_new, batch_action_new)
  ...
  return users_rf, all_list,answer,recommend, avg_q_list, reward_list, loss, precision, recall
...
users,users_rf,all_list, avg_q_list, reward_list =load_users(epochs)

loss_sum = []
precision = [[0]*50000 for _ in range(2)]
recall = [[0]*50000 for _ in range(2)]

for n_epi in range(epochs):
  #epsilon = max(0, 1 - ((1 / epochs) * n_epi))
  #stop epsilon 필요
  users_rf,all_list,answer,recommend, avg_q_list, reward_list,loss, precision, recall =  run(users,users_rf,all_list,n_epi,answer,recommend,agent,buffer,avg_q_list, reward_list, precision, recall)
  loss_sum.append(loss)

path = 'ddqn_model_8_nocontext_save.pth' #8
torch.save(agent.Q_network.state_dict(), path)

with open ("all_list_8_nocontext.pkl","wb") as f: #8
    pickle.dump(all_list,f)


with open ("users_rf_8_nocontext.pkl","wb") as g: #8
    pickle.dump(users_rf,g)

with open ("precision_8_nocontext.pkl","wb") as a: #8
    pickle.dump(precision,a)

with open ("recall_8_nocontext.pkl","wb") as b: #8
    pickle.dump(recall,b)
...
print("user 1's most prefer news = {}, {}".format(user1_result[7],user1_result[6]))
print("user 2's most prefer news = {}, {}".format(user2_result[7],user2_result[6]))
...
for i in range(2):
  colors=['y'] * 8
  if i==0:
    colors[user1_result[7]]='C2'
    colors[user1_result[6]]='C2'
    colors[4] = 'C2'
  elif i==1:
    colors[user2_result[7]]='C2'
    colors[user2_result[6]]='C2'
    colors[4] = 'C2'
