...
def run(users, users_rf,users_cf, all_list, n_epi,answer,recommend,agent,buffer,avg_q_list, reward_list,precision, recall):
  n=len(users)
  action1=[0]*n
  action2 = [0] *n
  action3 = [0] * n
...
#유저별로 카카오톡에서 읽은 뉴스들 카테고리 가져와서 학습돌리기
  ...
  return users_rf, users_cf, all_list,answer,recommend, avg_q_list, reward_list, precision, recall
...
for n_epi in range(epochs):
  ...
  #stop epsilon 필요
  users_rf,users_cf, all_list,answer,recommend, avg_q_list, reward_list, precision, recall =  run(users,users_rf,users_cf, all_list,n_epi,answer,recommend,agent,buffer,avg_q_list, reward_list,precision,recall)


path = 'ddqn_model_random_8.pth'
torch.save(agent.Q_network.state_dict(), path)

with open ("all_list_random_8.pkl","wb") as f:
    pickle.dump(all_list,f)


with open ("users_rf_random_8.pkl","wb") as g:
    pickle.dump(users_rf,g)

with open ("precision_8_random.pkl","wb") as a: #8
    pickle.dump(precision,a)

with open ("recall_8_random.pkl","wb") as b: #8
