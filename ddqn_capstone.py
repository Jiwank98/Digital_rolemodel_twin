...
def run(users, users_rf,all_list, n_epi,answer,recommend,agent,buffer,avg_q_list, reward_list):
  n=len(users)
  action1=[0]*n
  action2 = [0] *n
  action3 = [0] * n
...
#유저별로 카카오톡에서 읽은 뉴스들 카테고리 가져와서 학습돌리기
  ...
  return users_rf, all_list,answer,recommend, avg_q_list, reward_list
...
for n_epi in range(epochs):
  ...
  #stop epsilon 필요
  users_rf,all_list,answer,recommend, avg_q_list, reward_list =  run(users,users_rf,all_list,n_epi,answer,recommend,agent,buffer,avg_q_list, reward_list)


path = 'ddqn_model_IT_save.pth'
torch.save(agent.Q_network.state_dict(), path)

with open ("all_list_IT.pkl","wb") as f:
    pickle.dump(all_list,f)


with open ("users_rf_IT","wb") as g:
...
import matplotlib.pyplot as plt
import numpy as np
category=['Samsung','Kakao','Naver','MiddleStanding','StartUp']
for i in range(2):
  colors=['y'] * 5
  if i==0:
