...
class Config(object):
    lr = 0.001 #러닝레이트 조절하기 현재 0.0001 , 다음 0.001 , 다음 0.01, 다음 0.1, 다음 0.00001
...
#유저별로 카카오톡에서 읽은 뉴스들 카테고리 가져와서 학습돌리기
  ...
  return users_rf, users_cf, all_list,answer,recommend, avg_q_list, reward_list, loss
...
for n_epi in range(epochs):
  ...
  #stop epsilon 필요
  users_rf,users_cf, all_list,answer,recommend, avg_q_list, reward_list,loss =  run(users,users_rf,users_cf, all_list,n_epi,answer,recommend,agent,buffer,avg_q_list, reward_list)
  loss_sum.append(loss)

path = 'ddqn_model_loss_9.pth' #8-0.0001,9-0.001
torch.save(agent.Q_network.state_dict(), path)

with open ("all_list_loss_sum_9.pkl","wb") as f:
    pickle.dump(all_list,f) #8-0.0001,9-0.001

with open ("loss_sum_9.pkl","wb") as f:
    pickle.dump(loss_sum,f) #8-0.0001,9-0.001


with open ("users_rf_loss_sum_9.pkl","wb") as g:
    pickle.dump(users_rf,g) #8-0.0001,9-0.001
